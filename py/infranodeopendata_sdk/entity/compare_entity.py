...
from infranodeopendata_sdk.infranodeopendata_types import (
    Compare,
    CompareListMatch,
)
...
class CompareEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CompareEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Compare]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Compare().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
