# InfranodeOpenData SDK test runner
...
class InfranodeOpenDataTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            InfranodeOpenDataTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = InfranodeOpenDataTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = InfranodeOpenDataTestRunner.getenv("INFRANODE_OPEN_DATA_TEST_LIVE")
        override = InfranodeOpenDataTestRunner.getenv("INFRANODE_OPEN_DATA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = InfranodeOpenDataTestRunner.getenv(key)
        ...
        explain = InfranodeOpenDataTestRunner.getenv("INFRANODE_OPEN_DATA_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if InfranodeOpenDataTestRunner._test_control is not None:
            return InfranodeOpenDataTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                InfranodeOpenDataTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            InfranodeOpenDataTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = InfranodeOpenDataTestRunner.load_test_control()
...
def load_env_local():
    InfranodeOpenDataTestRunner.load_env_local()


def env_override(m):
    return InfranodeOpenDataTestRunner.env_override(m)


def entity_data(v):
    return InfranodeOpenDataTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return InfranodeOpenDataTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return InfranodeOpenDataTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return InfranodeOpenDataTestRunner.load_test_control()


def live_client_options():
    return InfranodeOpenDataTestRunner.live_client_options()


def live_delay_ms():
    return InfranodeOpenDataTestRunner.live_delay_ms()
