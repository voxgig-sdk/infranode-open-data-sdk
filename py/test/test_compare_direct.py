...
class TestCompareDirect:
    ...
    def test_should_direct_list_compare(self):
        setup = _compare_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-compare", "live" if setup["live"] else "unit")
        if _skip:
...
def _compare_direct_setup(mockres):
