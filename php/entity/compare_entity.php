...
// InfranodeOpenData SDK Compare entity
...
class CompareEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new CompareEntity($this->_client, $opts);
    }

    /**
     * @param Compare|array $args Compare data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = InfranodeOpenDataHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Compare|array The current Compare data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Compare fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = InfranodeOpenDataHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Compare fields).
     */
    ...
    /**
     * List Compare items matching the given filter.
     *
     * @param CompareListMatch|array|null $reqmatch Match filter (any subset
     *   of Compare fields) as an assoc-array; CompareListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Compare[]|array A list of Compare items as assoc-arrays at
     *   the SDK boundary; throws InfranodeOpenDataError on failure (item-5 convention).
     */
