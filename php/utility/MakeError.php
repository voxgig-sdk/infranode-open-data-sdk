...
// InfranodeOpenData SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class InfranodeOpenDataMakeError
{
    public static function call(?InfranodeOpenDataContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new InfranodeOpenDataContext([], null);
        }
        $op = $ctx->op ?? new InfranodeOpenDataOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new InfranodeOpenDataResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof InfranodeOpenDataError) ? $err->msg : (string)$err;
        $msg = "InfranodeOpenDataSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new InfranodeOpenDataError('', $msg, $ctx);
        ...
        if ($err instanceof InfranodeOpenDataError) {
