...
class InfranodeOpenDataSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = InfranodeOpenDataHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = InfranodeOpenDataHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!InfranodeOpenDataFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, InfranodeOpenDataFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return InfranodeOpenDataUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = InfranodeOpenDataHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = InfranodeOpenDataHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = InfranodeOpenDataHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new InfranodeOpenDataError($op . "_allow",
                "InfranodeOpenDataSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = InfranodeOpenDataHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new InfranodeOpenDataError("graphql_error",
                "InfranodeOpenDataSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    // Canonical facade: $client->Compare()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->compare()
    // resolves here too.
    public function Compare($data = null)
    {
        require_once __DIR__ . '/entity/compare_entity.php';
        if ($data === null) {
            if ($this->_compare === null) {
                $this->_compare = new CompareEntity($this, null);
            }
            return $this->_compare;
        }
        return new CompareEntity($this, $data);
    }
    ...
    // Canonical facade: $client->Live()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->live()
    // resolves here too.
    public function Live($data = null)
    {
        require_once __DIR__ . '/entity/live_entity.php';
        if ($data === null) {
            if ($this->_live === null) {
                $this->_live = new LiveEntity($this, null);
            }
            return $this->_live;
        }
        return new LiveEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new InfranodeOpenDataSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
