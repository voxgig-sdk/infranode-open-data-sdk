...
// InfranodeOpenData SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class InfranodeOpenDataTestFeature extends InfranodeOpenDataBaseFeature
{
    ...
    public function init(InfranodeOpenDataContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (InfranodeOpenDataContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = InfranodeOpenDataParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (InfranodeOpenDataContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(InfranodeOpenDataContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = InfranodeOpenDataParam::call($ctx, $k);
