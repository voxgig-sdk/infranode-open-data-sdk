...
class CityEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set INFRANODEOPENDATA_TEST_CITY_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $this->assertNotNull($city_ref01_data_dt0_loaded);
...
function city_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("INFRANODEOPENDATA_TEST_CITY_ENTID");
    $idmap_overridden = $entid_env_raw !== false && str_starts_with(trim($entid_env_raw), "{");

    $env = Runner::env_override([
        "INFRANODEOPENDATA_TEST_CITY_ENTID" => $idmap,
        "INFRANODEOPENDATA_TEST_LIVE" => "FALSE",
        "INFRANODEOPENDATA_TEST_EXPLAIN" => "FALSE",
    ]);

    $idmap_resolved = Helpers::to_map(
        $env["INFRANODEOPENDATA_TEST_CITY_ENTID"]);
