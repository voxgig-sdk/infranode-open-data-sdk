...
//    only - a typed InfranodeOpenDataContext there would make every ctx
...
final class InfranodeOpenDataOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new InfranodeOpenDataContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new InfranodeOpenDataSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new InfranodeOpenDataResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new InfranodeOpenDataError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new InfranodeOpenDataResponse($clean['response']);
    ...
    public static function err_from_map($m): ?InfranodeOpenDataError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new InfranodeOpenDataError(is_string($code) ? $code : '', $msg);
    }
