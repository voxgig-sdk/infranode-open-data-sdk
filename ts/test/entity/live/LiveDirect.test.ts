...
describe('LiveDirect', async () => {
  ...
  test('direct-load-live', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-live', setup.live)) return
    if (skipIfMissingIds(t, setup, ["live_id01","route_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['INFRANODE_OPEN_DATA_TEST_LIVE_ENTID']
