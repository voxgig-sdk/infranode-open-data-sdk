...
const __1 = require("..");
const SDK_NAME = 'InfranodeOpenDataSDK';
...
const TEST_SEED = { "entity": { "city": { "test01": { "id": "test01" } }, "compare": { "test01": { "id": "test01" } }, "health": { "test01": { "id": "test01" } }, "live": { "test01": { "id": "test01" } }, "meta": { "test01": { "id": "test01" } }, "station": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+InfranodeOpenDataSDK\b/.test(code) ||
        /\bInfranodeOpenDataSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+InfranodeOpenDataSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/InfranodeOpenDataSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
