...
func TestHealthEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set INFRANODEOPENDATA_TEST_HEALTH_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func healthBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("INFRANODEOPENDATA_TEST_HEALTH_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"INFRANODEOPENDATA_TEST_HEALTH_ENTID": idmap,
		"INFRANODEOPENDATA_TEST_LIVE":      "FALSE",
		"INFRANODEOPENDATA_TEST_EXPLAIN":   "FALSE",
	})

	idmapResolved := core.ToMapAny(env["INFRANODEOPENDATA_TEST_HEALTH_ENTID"])
