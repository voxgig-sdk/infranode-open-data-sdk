...
func TestStationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set INFRANODE_OPEN_DATA_TEST_STATION_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func stationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("INFRANODE_OPEN_DATA_TEST_STATION_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"INFRANODE_OPEN_DATA_TEST_STATION_ENTID": idmap,
		"INFRANODE_OPEN_DATA_TEST_LIVE":      "FALSE",
		"INFRANODE_OPEN_DATA_TEST_EXPLAIN":   "FALSE",
	})

	idmapResolved := core.ToMapAny(env["INFRANODE_OPEN_DATA_TEST_STATION_ENTID"])
