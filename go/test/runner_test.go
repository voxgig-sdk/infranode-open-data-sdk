...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("INFRANODEOPENDATA_TEST_LIVE") == "TRUE" ||
		os.Getenv("INFRANODEOPENDATA_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("INFRANODEOPENDATA_TEST_EXPLAIN"); explain != "" {
