...
import (
	...
	omni "github.com/voxgig-sdk/infranode-open-data-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.InfranodeOpenDataError); is {
				out := map[string]any{
					"name":    "InfranodeOpenDataError",
...
func omniCtx(arg any, client *sdk.InfranodeOpenDataSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.InfranodeOpenDataSDK); is {
			client = live
