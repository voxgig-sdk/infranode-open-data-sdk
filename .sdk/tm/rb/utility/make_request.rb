...
module InfranodeOpenDataUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = InfranodeOpenDataResponse.new({})
    ...
    if fetch_err
      ...
      response = InfranodeOpenDataResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = InfranodeOpenDataResponse.new(fetched)
    else
