...
// InfranodeOpenData SDK feature test
//
...
class FtCtrl extends InfranodeOpenDataControl
{
...
class FtClient
{
    ...
    private InfranodeOpenDataUtility $utility;

    public function __construct(InfranodeOpenDataUtility $utility, array $options)
    {
    ...
    public function get_utility(): InfranodeOpenDataUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (InfranodeOpenDataContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public InfranodeOpenDataContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (InfranodeOpenDataContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = InfranodeOpenDataConfig::make_feature($name);
    ...
    private static function buildUrl(InfranodeOpenDataSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof InfranodeOpenDataError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new InfranodeOpenDataResponse($fetched);
        }
    ...
    private function fail(InfranodeOpenDataContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (InfranodeOpenDataContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
