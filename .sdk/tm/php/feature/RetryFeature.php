...
// InfranodeOpenData SDK retry feature
...
class InfranodeOpenDataRetryFeature extends InfranodeOpenDataBaseFeature
{
    ...
    public function init(InfranodeOpenDataContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (InfranodeOpenDataContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(InfranodeOpenDataContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(InfranodeOpenDataContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
