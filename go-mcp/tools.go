...
type Args struct {
	Entity string         `json:"entity" jsonschema:"city | compare | health | live | meta | station"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.InfranodeOpenDataSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "infranode-open-data_list",
		Description: "List records from InfranodeOpenData. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "infranode-open-data_load",
		Description: "Load a single record from InfranodeOpenData. " +
...
func runOp(client *sdk.InfranodeOpenDataSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.InfranodeOpenDataSDK, name string) (sdk.InfranodeOpenDataEntity, error) {
	switch strings.ToLower(name) {
	case "city":
		return client.City(nil), nil
	case "compare":
		return client.Compare(nil), nil
	case "health":
		return client.Health(nil), nil
	case "live":
		return client.Live(nil), nil
	case "meta":
