# infranode-open-data-mcp

[MCP](https://modelcontextprotocol.io) server exposing the InfranodeOpenData SDK as
two agent tools — `infranode-open-data_list` and `infranode-open-data_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/infranode-open-data-mcp)
make build

# 2. Provide credentials via the environment
export INFRANODE_OPEN_DATA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user infranode-open-data \
  -- /absolute/path/to/infranode-open-data-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./infranode-open-data-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// infranode-open-data_list: first page of records
{ "entity": "city" }
{ "entity": "city", "query": { } }

// infranode-open-data_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/infranode-open-data-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user infranode-open-data \
     -- "$PWD"/dist/*/infranode-open-data-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `infranode-open-data_list` and `infranode-open-data_load` tools now appear
   in new sessions. Ask the agent to *"list city using infranode-open-data"*
   and it calls `infranode-open-data_list` with `{"entity":"city"}`.
...
```sh
export INFRANODE_OPEN_DATA_APIKEY=sk_live_xxx            # API key
export INFRANODE_OPEN_DATA_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `infranode-open-data_list` tool
...
### Call the `infranode-open-data_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `infranode-open-data_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `infranode-open-data_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `INFRANODE_OPEN_DATA_APIKEY` | API key sent with every request. |
| `INFRANODE_OPEN_DATA_BASE` | Optional override of the API base URL. |

### Entities

The 6 entities valid as the `entity` argument:

city | compare | health | live | meta | station

### Smoke test via HTTP (raw JSON-RPC)

```sh
./infranode-open-data-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"infranode-open-data_load","arguments":{"entity":"city","query":{"id":1}}}}'
```
