...
require("infranode-open-data_types")
...
local InfranodeOpenDataSDK = {}
InfranodeOpenDataSDK.__index = InfranodeOpenDataSDK
...
InfranodeOpenDataSDK._make_feature = _make_feature


function InfranodeOpenDataSDK.new(options)
  local self = setmetatable({}, InfranodeOpenDataSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function InfranodeOpenDataSDK:options_map()
...
function InfranodeOpenDataSDK:get_utility()
  return Utility.copy(self._utility)
end


function InfranodeOpenDataSDK:get_root_ctx()
  return self._rootctx
end


function InfranodeOpenDataSDK:prepare(fetchargs)
...
function InfranodeOpenDataSDK:direct(fetchargs)
...
function InfranodeOpenDataSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function InfranodeOpenDataSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "InfranodeOpenDataSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function InfranodeOpenDataSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function InfranodeOpenDataSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "InfranodeOpenDataSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function InfranodeOpenDataSDK:City(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function InfranodeOpenDataSDK:Compare(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function InfranodeOpenDataSDK:Health(data)
...
-- Idiomatic facade: client:Live():list() / client:Live():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function InfranodeOpenDataSDK:Live(data)
  local EntityMod = require("entity.live_entity")
  if data == nil then
    if self._live == nil then
      self._live = EntityMod.new(self, nil)
    end
    return self._live
  end
  return EntityMod.new(self, data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function InfranodeOpenDataSDK:Meta(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function InfranodeOpenDataSDK:Station(data)
...
function InfranodeOpenDataSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = InfranodeOpenDataSDK.new(sdkopts)
  sdk.mode = "test"
