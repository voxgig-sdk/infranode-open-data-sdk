...
function LiveEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LiveEntity)
  self._name = "live"
  self._client = client
...
function LiveEntity:get_name()
  return self._name
end


function LiveEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LiveEntity.new(self._client, opts)
end
...
function LiveEntity:mark_deleted()
  self._deleted = true
end


function LiveEntity:deleted()
  return true == self._deleted
end


function LiveEntity:data_set(args)
  if args ~= nil then
...
function LiveEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LiveEntity:match_set(args)
  if args ~= nil then
...
function LiveEntity:stream(action, args, callopts)
...
---@param reqmatch LiveLoadMatch
---@param ctrl? table
---@return Live
---@return string? err
function LiveEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function LiveEntity:_run_op(ctx, post_done)
  local utility = self._utility
