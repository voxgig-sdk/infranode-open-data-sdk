...
local CompareEntity = {}
CompareEntity.__index = CompareEntity


function CompareEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CompareEntity)
  self._name = "compare"
  self._client = client
...
function CompareEntity:get_name()
  return self._name
end


function CompareEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CompareEntity.new(self._client, opts)
end
...
function CompareEntity:mark_deleted()
  self._deleted = true
end


function CompareEntity:deleted()
  return true == self._deleted
end


function CompareEntity:data_set(args)
  if args ~= nil then
...
function CompareEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CompareEntity:match_set(args)
  if args ~= nil then
...
function CompareEntity:match_get()
...
function CompareEntity:stream(action, args, callopts)
...
---@param reqmatch CompareListMatch
---@param ctrl? table
---@return Compare[]
---@return string? err
function CompareEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CompareEntity:_run_op(ctx, post_done)
  local utility = self._utility
