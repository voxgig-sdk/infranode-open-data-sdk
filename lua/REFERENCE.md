# InfranodeOpenData Lua SDK Reference

Complete API reference for the InfranodeOpenData Lua SDK.
...
```lua
local results, err = client:Compare():list()
```
...
```lua
local result, err = client:Station():load({ eva = "eva" })
```
