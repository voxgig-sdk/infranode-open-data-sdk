# InfranodeOpenData Lua SDK



The Lua SDK for the InfranodeOpenData API — an entity-oriented client using Lua conventions.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/infranode-open-data-sdk/releases)),
...
Create an instance: `local compare = client:Compare(nil)`
...
```lua
local compares, err = client:Compare():list()
```
...
Create an instance: `local live = client:Live(nil)`
...
```lua
local station, err = client:Station():load({ eva = "eva" })
```
...
The main module (`infranode-open-data_sdk`) exports the SDK constructor
