// Command InfranodeOpenData-cli is a CLI + REPL for the InfranodeOpenData SDK,
...
const prompt = "infranode-open-data"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "city compare health live meta station"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      INFRANODE_OPEN_DATA_APIKEY (api key), INFRANODE_OPEN_DATA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: INFRANODE_OPEN_DATA_APIKEY carries the API key and
	// INFRANODE_OPEN_DATA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("INFRANODE_OPEN_DATA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("INFRANODE_OPEN_DATA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewInfranodeOpenDataSDK(opts)
