# infranode-open-data-cli

boru-driven command-line client **and** interactive REPL for the InfranodeOpenData
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/infranode-open-data-cli)
make build

# 2. See usage (words, entities, env vars)
./infranode-open-data-cli --help

# 3. Provide credentials once, via the environment
export INFRANODE_OPEN_DATA_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./infranode-open-data-cli list city
./infranode-open-data-cli load 1 city            # {id:1} shorthand
./infranode-open-data-cli load '{id:1}' city       # explicit match map
./infranode-open-data-cli list compare

# 5. Override the API base URL for a single call
INFRANODE_OPEN_DATA_BASE=https://api.example.com ./infranode-open-data-cli list city

# 6. No arguments -> interactive REPL
./infranode-open-data-cli
infranode-open-data> list city
infranode-open-data> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/infranode-open-data-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/infranode-open-data-cli list city
   ```
...
```sh
./infranode-open-data-cli load 1 city          # scalar shorthand for {id:1}
./infranode-open-data-cli load '{id:1}' city     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `infranode-open-data>`). Each line is
evaluated as its own boru expression:

```text
$ ./infranode-open-data-cli
infranode-open-data> list city
infranode-open-data> /help
infranode-open-data> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/infranode-open-data-cli`. |
...
(`./infranode-open-data-cli <expr>`) and interactively (the REPL), and expressions compose the
