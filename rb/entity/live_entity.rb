...
class LiveEntity
  ...
  # @return [Live, Hash] the current Live data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Live fields)
  def match_get
  ...
  # Load a single Live.
  #
  # @param reqmatch [LiveLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Live.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Live, Hash] the loaded Live; raises InfranodeOpenDataError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = InfranodeOpenDataHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
