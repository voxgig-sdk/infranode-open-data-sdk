...
class CompareEntity
  ...
  def make
    opts = @_entopts.dup
    CompareEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = InfranodeOpenDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Compare, Hash] the current Compare data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = InfranodeOpenDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Compare fields)
  def match_get
  ...
  #
  # @param reqmatch [CompareListMatch, Hash, nil] match filter (any subset of
  #   Compare fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Compare>, Array] the matching Compare items; raises InfranodeOpenDataError on failure
