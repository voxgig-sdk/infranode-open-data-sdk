...
class MetaEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Meta(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Meta(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set INFRANODEOPENDATA_TEST_META_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    meta_ref01_list_result = meta_ref01_ent.list(meta_ref01_match, nil)
    assert meta_ref01_list_result.is_a?(Array)
...
def meta_basic_setup(extra)
  ...
  entid_env_raw = ENV["INFRANODEOPENDATA_TEST_META_ENTID"]
  idmap_overridden = !entid_env_raw.nil? && entid_env_raw.strip.start_with?("{")

  env = Runner.env_override({
    "INFRANODEOPENDATA_TEST_META_ENTID" => idmap,
    "INFRANODEOPENDATA_TEST_LIVE" => "FALSE",
    "INFRANODEOPENDATA_TEST_EXPLAIN" => "FALSE",
  })

  idmap_resolved = Helpers.to_map(
    env["INFRANODEOPENDATA_TEST_META_ENTID"])
