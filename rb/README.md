# InfranodeOpenData Ruby SDK



The Ruby SDK for the InfranodeOpenData API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
require_relative "InfranodeOpenData_sdk"
client = InfranodeOpenDataSDK.new(options)
```
...
raise a `InfranodeOpenDataError` (a `StandardError` subclass), so wrap
...
```ruby
# list returns an Array of Compare records (raises on error).
...
```ruby
# list returns an Array of Meta records (raises on error).
...
The main module (`InfranodeOpenData_sdk`) exports the SDK class
