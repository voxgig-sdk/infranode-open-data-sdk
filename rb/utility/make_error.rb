...
module InfranodeOpenDataUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = InfranodeOpenDataContext.new({}, nil)
    end
    op = ctx.op || InfranodeOpenDataOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || InfranodeOpenDataResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(InfranodeOpenDataError) ? err.msg : err.to_s
    ...
    sdk_err = InfranodeOpenDataError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(InfranodeOpenDataError)
